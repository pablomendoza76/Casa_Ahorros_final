...
export class AprobadosComponent implements OnInit, AfterViewInit {
  prestamos: any[] = [];
  ...
  async ngOnInit(): Promise<void> {
    this.prestamos = await PrestamosMapper.obtenerPrestamosPorEstado(this.prestamoService, 'aceptado');
    console.log('📤 Datos enviados a la tabla dinámica:', this.prestamos);
    this.cdr.detectChanges(); // ✅ Fuerza la detección luego de asignar
  }
  ...
  ngAfterViewInit(): void {
    this.columnas = [
      ...
      { titulo: 'Aprobado el', tipo: 'texto', template: this.fechaTpl },
    ];
